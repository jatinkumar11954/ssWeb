...
NEWSCHEMA('Page').make(function(schema) {
	...
	schema.setGet(function($) {
		var opt = $.options;
		console.log("opttttttttttttttttttt", opt);
...
function refresh() {
	...
	NOSQL('pages').find().prepare(prepare).callback(function() {
		...
		//console.log("PAGES",F.global.pages, F.global.sitemap );
...
Controller.prototype.CMSpage = function(callback, cache) {
	...
	var self = this;
	//console.log("PAGES",F.global.pages );
	//console.log("SITEMAP",F.global.sitemap);
	var url = self.url.substr(6,self.url.length-1);
	// console.log("SUBSTRING",self.url.substr(6,self.url.length-1));
	// console.log("URL -----------------------------------------", url);
   ...
   // console.log("cms page ---------------------------------------", page);
	...
	self.memorize('cachecms' + url, cache || '1 minute', cache === false, function() {
		...
		nosql.one().where('id', page.id).callback(function(err, response) {
			...
			response.body.CMSrender(response.widgets, function(body) {
				response.body = body;
				repo.page = response;
				//console.log("repo.page ????????????????????",repo.page);
				loadpartial(repo.page, function(partial) {
					repo.page.partial = partial;
					// console.log("repo.page.partial---------------",repo.page.partial);
					// console.log("repo.page.template---------------",repo.page.template);
					if (callback) {
