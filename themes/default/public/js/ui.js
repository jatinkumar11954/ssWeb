...
COMPONENT('shoppingcart', 'discount:0;expiration:6 days', function(self, config) {
	...
	self.has = function(token) {
		if (token instanceof jQuery) {
			return self.has(HASH(token.attr('id') + token.attr('idvariant')).toString());
		} else {
...
COMPONENT('binder', function(self) {
	...
	self.scan = function() {
		...
		self.find('[data-b]').each(function() {
			...
			var el = $(this);
			var path = el.attr('b').replace('%', 'jctmp.');
			...
			var classes = el.attr('b-class');
			var html = el.attr('b-html');
			var visible = el.attr('b-visible');
			var disable = el.attr('b-disable');
			var selector = el.attr('b-selector');
			var src = el.attr('b-src');
			var href = el.attr('b-href');
...
COMPONENT('modificator', function(self) {
	...
	self.make = function() {
		...
		$(document).on('click', '.modify', function() {
			var el = $(this);
			self.click(el.attr('m'), el.attr('m-schema'));
		});
	};
	...
	self.scan = function() {
		...
		self.find('[data-m]').each(function() {
			...
			var el = $(this);
			var path = (el.attr('m') || '').replace('%', 'jctmp.');
			var p = '';
			var schema = '';
			...
			if (!obj) {
				obj = {};
				obj.path = path;
				obj.schema = schema || el.attr('m-schema');
...
COMPONENT('autocomplete', 'height:200', function(self, config) {
	...
	self.make = function() {
		...
		self.event('click', 'li', function(e) {
			e.preventDefault();
			e.stopPropagation();
			onCallback && onCallback(datasource[+$(this).attr('index')], old);
			self.visible(false);
		});
